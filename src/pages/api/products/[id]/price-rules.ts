...
import { priceRules } from '@/lib/db/schema';
...
export const PUT: APIRoute = async (context) => {
    ...
    try {
        ...
        const rules: Array<{ minQuantity: number; maxQuantity: number | null; discountPercentage: number | string }> = body.rules || [];
        ...
        for (let i = 0; i < rules.length; i++) {
            ...
            if (!ruleA.discountPercentage || isNaN(parseFloat(ruleA.discountPercentage as string))) {
                return new Response(JSON.stringify({ error: `El porcentaje de descuento es requerido y debe ser un número (rango ${i + 1})` }), { status: 400 });
            }
        ...
        await db.transaction(async (tx) => {
            await tx.delete(priceRules).where(eq(priceRules.productId, productId));
            ...
            if (rules.length > 0) {
                await tx.insert(priceRules).values(
                    rules.map(r => ({
                        ...
                        discountPercentage: parseFloat(r.discountPercentage as string).toString(),
                    }))
                );
            }
        });
        ...
        console.error('Error saving price rules:', e);
        return new Response(JSON.stringify({ error: 'Error al guardar reglas de descuento' }), { status: 500 });
    }
};
